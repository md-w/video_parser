...
using number_type = typename std::conditional<
    ...
    typename std::conditional<
        ...
        typename std::conditional<sizeof(T) == 4, SizeOf4,
                                  typename std::conditional<sizeof(T) == 8, SizeOf8, UnknownSize>::type>::type>::type>::
    type;
...
class file_istream {
 public:
  ...
  file_istream(const char* file) : input_file_ptr(nullptr), file_size(0L), read_length(0L) { open(file); }
#ifdef _MSC_VER
  file_istream(const wchar_t* file) : input_file_ptr(nullptr), file_size(0L), read_length(0L) { open(file); }
#endif
  ~file_istream() { close(); }
...
class mem_istream {
 public:
  ...
  template <typename T>
  void read(T& t) {
    if (eof()) throw std::runtime_error("Premature end of array!");

    if ((m_index + sizeof(T)) > m_vec.size()) throw std::runtime_error("Premature end of array!");
  ...
  void read(typename std::vector<char>& vec) {
    ...
    if ((m_index + vec.size()) > m_vec.size()) throw std::runtime_error("Premature end of array!");
  ...
  void read(char* p, size_t size) {
    ...
    if ((m_index + size) > m_vec.size()) throw std::runtime_error("Premature end of array!");
...
class ptr_istream {
 public:
  ...
  ptr_istream(const char* mem, size_t size) : m_arr(nullptr), m_size(0), m_index(0) { open(mem, size); }
  ...
  void read(T& t) {
    ...
    if ((m_index + sizeof(T)) > m_size) throw std::runtime_error("Premature end of array!");
  ...
  void read(typename std::vector<char>& vec) {
    ...
    if ((m_index + vec.size()) > m_size) throw std::runtime_error("Premature end of array!");
  ...
  void read(char* p, size_t size) {
    ...
    if ((m_index + size) > m_size) throw std::runtime_error("Premature end of array!");
...
class memfile_istream {
 public:
  ...
  memfile_istream(const char* file) : m_arr(nullptr), m_size(0), m_index(0) { open(file); }
#ifdef _MSC_VER
  memfile_istream(const wchar_t* file) : m_arr(nullptr), m_size(0), m_index(0) { open(file); }
#endif
...
class file_ostream {
 public:
  ...
  file_ostream(const char* file) : output_file_ptr(nullptr) { open(file); }
#ifdef _MSC_VER
  file_ostream(const wchar_t* file) : output_file_ptr(nullptr) { open(file); }
#endif
  ~file_ostream() { close(); }
  void open(const char* file) {
    close();
  ...
  void write(const char* p, size_t size) { std::fwrite(reinterpret_cast<const void*>(p), size, 1, output_file_ptr); }
...
class mem_ostream {
 public:
  ...
  void write(const char* p, size_t size) {
    for (size_t i = 0; i < size; ++i) m_vec.push_back(p[i]);
  }
  template <typename T>
  ...
  void writeat(size_t pos, const std::vector<char>& vec) {
    for (size_t n = 0, count = vec.size(); n < count; n++) m_vec[pos++] = vec[n];
  }
